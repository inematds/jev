...
**Ponto de integração:** Insira a avaliação entre a criação do chamado e a escolha da fila.

**Uso da saída:** Criar uma sugestão de fila no chamado; manter o atendimento humano disponível.
...
python3 -m pacotes.executar atendimento --state-file /caminho/entrada.txt --live --provider openrouter
```
...
No exemplo, a classificação esperada é **suporte**. O valor de confidence da fixture é inventado. A alternativa `revisar` e as alternativas de insuficiência continuam sob revisão mesmo com confidence alta. Sugestões não executam ações por conta própria.
