...
**Ponto de integração:** Faça a extração de texto antes desta etapa e envie o checklist junto.

**Uso da saída:** Apontar pendência para conferência, sem validar autenticidade ou conformidade jurídica.
...
python3 -m pacotes.executar documentos

# Dados próprios em texto UTF-8; chamada real à API
python3 -m pacotes.executar documentos --state-file /caminho/entrada.txt --live --provider openrouter
```
...
No exemplo, a classificação esperada é **incompleto**. O valor de confidence da fixture é inventado. A alternativa `revisar` e as alternativas de insuficiência continuam sob revisão mesmo com confidence alta. Sugestões não executam ações por conta própria.
