...
if(['127.0.0.1','localhost'].includes(location.hostname))fetch('/api/status').then(r=>r.json()).then(s=>{if(s.local){liveAvailable=s.configured;$('mode').textContent=s.configured?'Servidor local conectado · exemplos simulados; consultar envia o contexto à TypeSafe.':'Servidor local conectado · chave não configurada. Exemplos disponíveis.';$('live').hidden=!liveAvailable}}).catch(()=>{});
